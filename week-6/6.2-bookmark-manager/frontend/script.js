...
const addBookmarkToDOM = (bookmark) => {
  ...
  let clutter = `
        <li class='bookmark-item'>
            <span> ${bookmark?.bookmarkUrl} (${
    bookmark.bookmarkCategory
  }) </span>
            <button class='dlt-btn' onclick='deleteBookmark(${
              bookmark.id
            })'>Delete</button>
            <button class='fvrt-btn ${
              bookmark.isFavorite ? "fav" : ""
  ...
  bookmarkList.innerHTML += clutter;
};
...
document
  ...
  .addEventListener("click", async () => {
    ...
    const bookmarkCategory = bookMarkCategoryInput.value;
    ...
    const response = await fetch(API_URL, {
      ...
      body: JSON.stringify({ bookmarkUrl, bookmarkCategory }),
    });
    ...
    if (data) {
      ...
      bookMarkUrlInput.blur();
      bookMarkCategoryInput.value = "";
      bookMarkCategoryInput.blur();
      fetchBookmarks();
    } else {
      alert("Error");
    }
...
// toggleFavorite a bookmark
const toggleFavorite = async (id) => {
  const response = await fetch(`${API_URL}/toggleFavorite/${id}`);
  const data = await response.json();

  if (data) {
    alert("Bookmark Favorite Toggled Successfully.");
    fetchBookmarks();
  } else {
    alert("Error");
  }
